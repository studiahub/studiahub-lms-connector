...
- **Combos** (un producto WC = N cursos): inscriben en todos sus cursos sin importar cómo nació el pedido — checkout del front, alta manual en wp-admin (el flujo de la transferencia bancaria), REST API o un ERP. Los course IDs se guardan en el order item al crearlo y, si igual faltaran, se resuelven al armar el payload — así que un pedido viejo también se recupera con "entregar de nuevo" o con la reconciliación. En una moneda que no sea la base, un combo **solo** se vende si el dueño le cargó un precio fijo para esa moneda: nunca se cobra una conversión por cotización (ver [docs/multimoneda-y-troubleshooting.md](docs/multimoneda-y-troubleshooting.md)).
- Expone `GET /wp-json/studiahub/v1/health` para test de conexión desde el LMS.
- Red de seguridad del revenue path: expone `GET /wp-json/studiahub/v1/orders/recent` para que el cron de reconciliación del LMS vaya a buscar los pedidos `completed` cuyo webhook nunca llegó (LMS caído, entrega perdida, webhook pausado). Devuelve el mismo payload que el webhook, así que el LMS los procesa por el camino de siempre.
- Cierre de inscripciones real: un curso en preventa (`comingSoon`) o con la venta cerrada (`salesEndAt` vencido) no se puede agregar al carrito ni pagar, tampoco entrando por `?add-to-cart=` o por un link viejo del checkout.
